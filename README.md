<h1 align="center">João Carlos | Full Stack Developer</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1000&color=2F80ED&center=true&vCenter=true&width=750&lines=Desenvolvedor+Full+Stack+focado+em+Node.js+e+React;Cria%C3%A7%C3%A3o+de+APIs+escal%C3%A1veis+e+sistemas+web;Arquitetura+de+backends+modernos+com+TypeScript;Constru%C3%A7%C3%A3o+de+produtos+digitais+reais" />
</p>

---

## Perfil 

Desenvolvedor Full Stack com foco em **Node.js, TypeScript e React**, atuando na construção de sistemas web completos, APIs escaláveis e aplicações modernas.

Tenho experiência prática em arquitetura de backend, integração de sistemas e modelagem de dados, com atenção a boas práticas e organização de código.

Busco oportunidades como **Desenvolvedor Júnior ou Estágio**, com foco em evolução contínua e participação em produtos reais.

---

## Stack Principal

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nodejs,typescript,javascript,html,css" />
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=postgres,docker,git,linux,python" />
</p>

---

## Projetos Relevantes

### 🏆 Esportefull – Plataforma de Gestão Esportiva
Sistema completo para gerenciamento de competições, times e instituições esportivas.

🔗 Repositório: https://github.com/joao4692/Esportefull

**Descrição técnica:**
Plataforma desenvolvida com foco em organização de dados esportivos e estruturação de regras de negócio.

**Responsabilidades técnicas:**
- Estruturação de backend  
- Modelagem de dados e entidades  
- Regras de negócio para competições  
- Organização de fluxo de informações  

**Destaques:**
- Sistema escalável  
- Estrutura modular  
- Foco em backend e lógica de aplicação  

---

### ⚙️ Projetos em evolução
- APIs REST com Node.js + TypeScript  
- Autenticação JWT e segurança de aplicações  
- Integração com PostgreSQL  
- Interfaces web com React  
- Deploy de aplicações com Docker  

---

## Arquitetura & Princípios

- Código limpo e organizado  
- Separação de responsabilidades  
- Estrutura escalável de backend  
- Foco em performance e manutenção  
- Boas práticas de API design  

---

## GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=default" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=SEU_USUARIO&theme=default" />
</p>

---

## Contato Profissional

- LinkedIn: https://www.linkedin.com/in/joao-carlos-leite-da-silva-452319333  
- WhatsApp: https://wa.me/5581987033735  
- GitHub: https://github.com/SEU_USUARIO  

---

<p align="center">
  <b>Disponível para oportunidades como Desenvolvedor Júnior ou Estágio em Desenvolvimento Web.</b>
</p>
